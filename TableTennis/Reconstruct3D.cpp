...
void calibrateCamera(CvPoint2D32f *ptsLeft, CvPoint2D32f *ptsRight, CvSize imageSize, CvMat *intrinsicMatrix, CvMat *distortionMatrix, CvMat *rotationVectors, CvMat *translationVectors){
	CvMat *objectPoints = cvCreateMat(8, 3, CV_32F);
	CvMat *imagePoints = cvCreateMat(8, 2, CV_32F);
	CvMat *pointCounts = cvCreateMat(2, 1, CV_8U);
	for (int i = 0; i < 4; ++i){
	...
	*((char*)CV_MAT_ELEM_PTR(*pointCounts, 0, 0)) = 4;
	*((char*)CV_MAT_ELEM_PTR(*pointCounts, 1, 0)) = 4;
